# microgrid-experiment-setup
Experimental setup and supplementary materials for microgrid energy management study

# Microgrid Experimental Setup
This repository provides the real laboratory setup used in the study on microgrid energy management.

The setup includes:
- Photovoltaic system (PV)
- Battery storage
- Smart grid testbench
- PHIL-based experimental platform

## Figure
The experimental setup image is provided in this repository for reference.
